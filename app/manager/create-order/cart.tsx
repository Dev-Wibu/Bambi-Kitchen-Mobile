...
export default function CreateOrderCartScreen() {
  ...
  const handleItemClick = (item: (typeof items)[0]) => {
    ...
    if (item.recipe && item.recipe.length > 0) {
      ...
      if (item.basedOnId && item.basedOnId > 0) {
        router.push(`/manager/create-order/${item.basedOnId}`);
      } else {
        // Fully custom dish, go to customize page
        router.push(`/manager/create-order/customize`);
      }
    } else if (item.dishId && item.dishId > 0) {
      // Navigate to dish detail page for preset dishes
      router.push(`/manager/create-order/${item.dishId}`);
    }
  };

  const placeOrder = async () => {
    // NOTE: Staff creates orders for walk-in customers
    // Backend requires accountId - use staff account ID for counter orders
    if (!user?.userId) {
      Toast.show({ type: "error", text1: "Please login to create order" });
      return;
    }
    if (isEmpty) {
    ...
    try {
      ...
      if (__DEV__) {
        ...
        console.log("Staff creating order with accountId:", user.userId);
      }

      // Use staff account ID for counter orders (backend requirement)
      const payload = transformCartToMakeOrderRequest({
        accountId: user.userId, // Staff account creates order for walk-in customer
        paymentMethod,
        note,
        items,
      });
