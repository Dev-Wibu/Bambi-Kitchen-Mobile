...
export default function DishForm({ visible, dish, onClose, onSuccess }: DishFormProps) {
  ...
  // Handle response wrapping - API might return { data: [...] } or just [...]
  ...
  const availableIngredients = React.useMemo(() => {
    ...
    console.log("🔍 [DishForm] Ingredients API Response:", {
      type: typeof ingredientsResponse,

      isArray: Array.isArray(ingredientsResponse),

      hasData: ingredientsResponse && "data" in ingredientsResponse,

      keys: ingredientsResponse ? Object.keys(ingredientsResponse) : [],

      sample: ingredientsResponse,
    });
    ...
    if (
      ingredientsResponse &&
      typeof ingredientsResponse === "object" &&
      "data" in ingredientsResponse
    ) {
      const unwrapped = (ingredientsResponse as any).data;

      console.log("✅ [DishForm] Unwrapped ingredients count:", unwrapped?.length || 0);
    ...
    console.log("✅ [DishForm] Direct array ingredients count:", ingredientsResponse.length || 0);

    return Array.isArray(ingredientsResponse) ? ingredientsResponse : [];
  }, [ingredientsResponse]);
  ...
  const handleSubmit = async () => {
    console.log("🚀 [DishForm] Submit started", {
      name,
    ...
    console.log("✅ [DishForm] Valid ingredients:", validIngredients);
    ...
    try {
      ...
      if (file && file.uri) {
        ...
        console.log("⚠️ [DishForm] No new file selected, sending empty file placeholder");
      ...
      const ingredientsToSend = Object.entries(ingredients).filter(([, qty]) => qty > 0);

      console.log("📦 [DishForm] Ingredients to send:", ingredientsToSend);

      ingredientsToSend.forEach(([key, value]) => {
        formData.append(`ingredients[${key}]`, String(value));

        console.log(`   - ingredients[${key}] = ${value}`);
      });

      if (id) formData.append("id", String(id));

      console.log("🌐 [DishForm] Sending request to backend...");
      ...
      console.log("📡 [DishForm] Response status:", response.status);

      if (!response.ok) {
        const errorText = await response.text();

        console.error("❌ [DishForm] Error response:", errorText);
      ...
      console.log("✅ [DishForm] Success response:", responseData);
        ...
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.5)", justifyContent: "center" }}>
          <View className="mx-4 max-h-[80%] rounded-xl bg-white p-4">
            ...
            {loadingIngredients ? (
              ...
              <View>
                <Text className="text-center text-red-500">⚠️ No ingredients available</Text>
